...
class Game:
    def __init__(self, start_t: float, end_t: float, victor_count: int, results: list[(str, int)], name: str):
        ...
        results_copy: list[(str, int)] = results.copy()
        results_copy.sort(lambda t: t[1], True)
...
game_files = [file_name for file_name in os.listdir(games_location) if os.path.isfile(file_name)]
...
for path in game_files:
    ...
    with open(path) as f:
        ...
        if len(lines) <= 4:
            error("Error: too few lines in {path}. Each file must at least have 3 lines for the "
        ...
        try:
            ...
            error("Error: header in file {path} was not formatted correctly. Files need a header "
        ...
        for line in lines:
            ...
            for o_p, _ in vsccs:
                if o_p == original_player:
                    error("Error: file {path} contained two nations played by the same player.")
            try:
                score = int(score)
            except Exception:
                error("Error: file {path} line {line_count}'s score (third item) was not a number. "
                      "Each line after the header should contain the original player discord tag,"
                      "final player discord tag, and vscc%, in that order, with tabs between them.")
...
the_time: int = -1
games.sort(lambda g: g.start_t * 10000)
while True:
    the_time += 1
